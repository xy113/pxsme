...
var dictionaryUrl = 'http://www.songdewei.com/';
// other options include:
// var dictionaryUrl = 'http://www.ninjawords.com/'; var dictionaryUrl = 'http://www.dictionary.com/';
var edButtons = new Array();var edLinks = new Array();var edOpenTags = new Array();
...
edButtons.push( new edButton('ed_link','link','','</a>','a') ); // special case
// edButtons.push(new edButton('ed_ext_link','Ext. Link','','</a>','e') ); // special case
...
edButtons.push( new edButton('ed_block','blockq','<blockquote>','</blockquote>','q') );
...
edButtons.push(new edButton('ed_footnote','Footnote','','','f'));
edButtons.push(new edButton('ed_via','Via','','','v'));
function edLink(display, URL, newWin) {this.display = display;this.URL = URL;if (!newWin) {newWin = 0;}this.newWin = newWin;}
edLinks[edLinks.length] = new edLink('webjcw.cn','http://www.webjcw.cn/');
function edShowButton(which, button, i) {if (button.access) {var accesskey = ' accesskey = "' + button.access + '"'}
else {var accesskey = '';}switch (button.id) {case 'ed_img':
...
function edShowLinks() {
	...
	for (i = 0; i < edLinks.length; i++) {tempStr += '<option value="' + i + '">' + edLinks[i].display + '</option>';}tempStr += '</select>';document.write(tempStr);}
...
function edCheckOpenTags(which, button) {var tag = 0;for (i = 0; i < edOpenTags[which].length; i++) {if (edOpenTags[which][i] == button) {tag++;}}if (tag > 0) {return true; // tag found
...
function edQuickLink(i, thisSelect) {if (i > -1) {var newWin = '';if (edLinks[i].newWin == 1) {newWin = ' target="_blank"';}var tempStr = '<a href="' + edLinks[i].URL + '"' + newWin + '>' 
		            + edLinks[i].display
		            + '</a>';thisSelect.selectedIndex = 0;edInsertContent(edCanvas, tempStr);}else {thisSelect.selectedIndex = 0;}}
...
function edToolbar(which) {
	...
	if (edShowExtraCookie()) {
		document.write(
		//	'<input type="button" id="ed_close_' + which + '" class="ed_button" onclick="edCloseAllTags(\'' + which + '\');" value="CloseT" />'
